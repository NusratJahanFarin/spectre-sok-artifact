...
void profile_leakByteLocalFunction(uint8_t k) {
   temp &= publicarray2[k * 512];
}
void profile_function_v02(size_t x) {
   if (x < publicarray_size) {
       profile_leakByteLocalFunction(publicarray[x]);
   }
}
